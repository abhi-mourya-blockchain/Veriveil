...
export class MidnightWalletProvider implements MidnightProvider, WalletProvider {
  ...
  static async build(logger: Logger, env: EnvironmentConfiguration, seed?: string): Promise<MidnightWalletProvider> {
    ...
    const trimmedSeed = seed ? seed.trim() : undefined;
    const seeds = trimmedSeed
      ? (trimmedSeed.includes(' ') && typeof (WalletSeeds as any).fromMnemonic === 'function'
          ? (WalletSeeds as any).fromMnemonic(trimmedSeed)
          : WalletSeeds.fromMasterSeed(trimmedSeed))
