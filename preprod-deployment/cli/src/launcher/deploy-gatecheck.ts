...
async function main() {
  ...
  const dustTx = await generateDust(logger, seed, unshieldedState, walletProvider.wallet, walletProvider.unshieldedKeystore);
  if (dustTx) {
  ...
  try {
    ...
    try {
      fs.writeFileSync(path.resolve(process.cwd(), '../../deployed_contract.json'), JSON.stringify(deploymentInfo, null, 2));
    } catch {}
    try {
      fs.writeFileSync(path.resolve(process.cwd(), 'deployed_contract.json'), JSON.stringify(deploymentInfo, null, 2));
    } catch {}
